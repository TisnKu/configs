...
vim.api.nvim_create_user_command("InstallCocExtensions", function()
  local extensions = {
    "coc-sumneko-lua",
    "coc-tsserver",
    "coc-json",
  ...
  for _, ext in ipairs(extensions) do
    if not vim.fn.system("CocList extensions --list-installed"):find(ext, 1, true) then
      print("Installing extension: " .. ext)
      vim.cmd("CocInstall " .. ext)
    else
      print("Extension already installed: " .. ext)
    end
  end
end, {
...
keyset("n", "[c", "<Plug>(coc-git-prevchunk)", { silent = true })
keyset("n", "]c", "<Plug>(coc-git-nextchunk)", { silent = true })
...
--keyset("i", "<C-y>", [[coc#pum#visible() ? coc#pum#confirm() : "\<C-g>u\<C-y>\<c-r>=coc#on_enter()\<CR>"]], opts)
...
keyset("x", "<Bslash>f", "<Plug>(coc-format-selected)", { silent = true })
keyset("n", "<Bslash>f", "<Plug>(coc-format-selected)", { silent = true })
